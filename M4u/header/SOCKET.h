...
public:
    void init();
    SocketObj(const char* ip, short port, int af, int sockTyp);
    SocketObj(){}
...
// open socket with the specified data
void SocketObj::open(const char* ip, short port, int af, int sockTyp)
{
    addressFamily = af;
    socketType = sockTyp;
    PORT = port;
